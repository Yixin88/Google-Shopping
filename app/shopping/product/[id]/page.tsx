    ...
    <div className="p-12 pt-0">
      ...
      {productData.content.reviews && (
        <div className="flex space-x-1">
          {/* Yellow rating stars */}
          {[...Array(Math.round(productData.content.reviews.rating))].map((_, i) => (
            <StarIcon key={i} className="h-5 w-5 text-yellow-500" />
          ))}
    
          {/* Grey rating stars */}
          {[...Array(5 - Math.round(productData.content.reviews.rating))].map((_, i) => (
            <StarIcon key={i} className="h-5 w-5 text-grey-200" />
          ))}
        </div>
      )}
            ...
            {productData.content.reviews.top_review && (
              <div className="border p-5 rounded-lg mt-2">
                ...
                <div className="flex space-x-1 mb-2">
                  {[...Array(Math.round(productData.content.reviews.top_review.rating))].map((_, i) => (
                    <StarIcon key={i} className="h-5 w-5 text-yellow-500" />
                  ))}
                </div>
